...
public final class ParticlePacketReceiver implements ClientPacketReceiver<ParticlePayload> {
	...
	public void handlePayload(ParticlePayload particlePayload, ClientPlayNetworking.Context context) {
		context.client().execute(() -> Objects.requireNonNull(context.client().world).addParticleClient(NetworkingConstants.ParticlePacketConstants.getGeneratorById(particlePayload.id()).get(context.client().world, particlePayload.x(), particlePayload.y(), particlePayload.z()), particlePayload.x(), particlePayload.y(), particlePayload.z(), particlePayload.dx(), particlePayload.dy(), particlePayload.dz()));
	}
