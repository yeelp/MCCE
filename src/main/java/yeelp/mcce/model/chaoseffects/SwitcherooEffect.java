...
public final class SwitcherooEffect extends AbstractInstantChaosEffect {
	...
	protected boolean isApplicableIgnoringStackability(PlayerEntity player) {
		return (int) player.getHealth() != player.getHungerManager().getFoodLevel() && player.getHungerManager().getFoodLevel() > 0 && !MCCEAPI.accessor.isChaosEffectActive(player, ChaosEffects.SUDDEN_DEATH);
	}
