...
public final class IOUEffect extends AbstractTimedChaosEffect {
	...
	private static final int DURATION_MIN = 4000, DURATION_MAX = 4500;
	public IOUEffect() {
	...
	private static final class BlockDropChanger implements ModifyBlockDrops {
		...
		static {
			...
			NETHER_ORES.add(Blocks.NETHER_GOLD_ORE);
		...
		private static ItemStack getIOU(String oreName) {
			...
			stack.set(DataComponentTypes.CUSTOM_NAME, Text.empty().append("IOU").formatted(Formatting.BOLD).formatted(Formatting.RED));
		...
		private static List<Text> getTextWithColor(String text, Formatting formatting) {
			return Text.of(text).getWithStyle(Style.EMPTY.withColor(formatting));
		}
	}
