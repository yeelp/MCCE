...
public final class ChaosLib {
     ...
     * @param arr the array.
     * @param <T> The type of elements in the array
     * @return A random element from the array. This is not a copy, so changes to the returned element will change the element in the array.
     */
    public static <T> T getRandomElementFrom(T[] arr) {
        return getRandomElementFrom(arr, ChaosLib.getStaticRandomInstance());
    }
